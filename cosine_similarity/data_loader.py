...
def load_test_data(args, logger, processor, task_name, label_list, tokenizer, output_mode, k=None):
    ...
    if args.model_type in ["MELBERT_MIP", "MELBERT"]:
        ...
        all_sentence_masks = torch.tensor([f.sentence_mask for f in eval_features], dtype=torch.long)
        all_word_ids = torch.tensor([f.word_index for f in eval_features], dtype=torch.long)

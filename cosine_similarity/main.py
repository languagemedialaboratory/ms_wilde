...
def main():
    ...
    args = config
    if args.use_cls.lower() == "none" and args.use_average.lower() != "both":
        raise ValueError("Invalid combination for parameters use_average and use_cls")
    elif args.use_cls.lower() != "both" and args.use_average.lower() == "none":
        raise ValueError("Invalid combination for parameters use_average and use_cls")
    elif args.use_cls.lower() == args.use_average.lower() and args.use_average.lower() != "both":
        raise ValueError("Invalid combination for parameters use_average and use_cls")
    print(args.__dict__)
    ...
    for eval_batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        with torch.no_grad():
            ...
            if args.model_type in ["BERT_BASE", "BERT_SEQ", "MELBERT_SPV"]:
                ...
                # logits,target_output_2,target_output,pooled_output = model(
                logits = model(
                    input_ids,
                    input_ids_2,
                    target_mask=(segment_ids == 1),
                ...
                # sim_label_ids = (2*label_ids-1)*(-1)
                # loss_cosine_MIP = nn.CosineEmbeddingLoss()
                # loss_cosine_SPV = nn.CosineEmbeddingLoss()
                # loss_MIP = loss_cosine_MIP(target_output_2,target_output,sim_label_ids.view(-1).float())
                # loss_SPV = loss_cosine_SPV(pooled_output,target_output,sim_label_ids.view(-1).float())

                # tmp_eval_loss += 0.5*loss_MIP + 0.5*loss_SPV

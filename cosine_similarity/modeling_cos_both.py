...
class AutoModelForSequenceClassification_SPV_MIP(nn.Module):
    ...
    def __init__(self, args, Model, config, num_labels=2):
        ...
        self.SPV_linear = nn.Linear(config.hidden_size * 2+1, args.classifier_hidden)
        self.MIP_linear = nn.Linear(config.hidden_size * 2+1, args.classifier_hidden)
    ...
    def forward(
        ...
        if self.args.use_average.lower() in spv_arg and self.args.use_cls.lower() in no_spv_arg:
            pooled_output = sequence_output * main_sentence_mask.unsqueeze(2)
            pooled_output = self.dropout(pooled_output)
            pooled_output = pooled_output.mean(1)
        elif self.args.use_average.lower() in spv_arg and self.args.use_cls.lower() in spv_arg:
            pooled_output = outputs[1]  # [batch, hidden]
            pooled_output = self.dropout(pooled_output)
            pooled_output = pooled_output.mean(1)
        elif self.args.use_average.lower() in no_spv_arg and self.args.use_cls.lower() in spv_arg:
            pooled_output = outputs[1]  # [batch, hidden]
        ...
        if self.args.use_average.lower() in mip_arg and self.args.use_cls.lower() in no_mip_arg:
            target_output_2 = sequence_output_2 * target_mask_2.unsqueeze(2)
            target_output_2 = self.dropout(target_output_2)
            target_output_2 = target_output_2.mean(1)
        elif self.args.use_average.lower() in mip_arg and self.args.use_cls.lower() in mip_arg:
            target_output_2 = outputs_2[1]
            target_output_2 = self.dropout(target_output_2)
            target_output_2 = target_output_2.mean(1)
        elif self.args.use_average.lower() in no_mip_arg and self.args.use_cls.lower() in mip_arg:
            target_output_2 = outputs_2[1]
        ...
        if labels is not None:
            ...
            # labels = (2*labels-1)*(-1)
            # loss_cosine_MIP = nn.CosineEmbeddingLoss()
            # loss_cosine_SPV = nn.CosineEmbeddingLoss()
            # loss_MIP = loss_cosine_MIP(target_output_2,target_output,labels.view(-1).float())
            # assert loss_MIP > 0
            # loss_SPV = loss_cosine_SPV(pooled_output,target_output,labels.view(-1).float())
            # assert loss_SPV > 0

            # loss = loss_bert + 0.5*loss_MIP + 0.5*loss_SPV
            return loss
        return logits #, target_output_2,target_output,pooled_output

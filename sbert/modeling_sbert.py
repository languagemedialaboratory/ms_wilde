...
s_bert = SentenceTransformer("roberta-base-nli-stsb-mean-tokens")
...
class AutoModelForSequenceClassification_SPV_MIP(nn.Module):
    ...
    def forward(
        ...
        #     context_sentence.append(token)
        # target2 = s_bert.encode(context_sentence)
        # target2 = torch.tensor(target2, requires_grad=False, dtype=torch.float).cuda()
        # target_output = target_output.add(target2)
        ...
        for input_id in input_list:
            token = tokenizer.decode(input_id, skip_special_tokens=True)
            tokens.append(token)
        # pooled_output2 = s_bert.encode(tokens)
        # pooled_output2 = torch.tensor(pooled_output2, requires_grad=False, dtype=torch.float).cuda()
        # pooled_output = pooled_output.add(pooled_output2)
        pooled_output = s_bert.encode(tokens)
        ...
        for input_id in input_list:
            token = tokenizer.decode(input_id, skip_special_tokens=True)
            tokens.append(token)
        definition_output = s_bert.encode(tokens)
        definition_output = torch.tensor(definition_output, requires_grad=False, dtype=torch.float).cuda()
        # target_output_2 = target_output_2.add(definition_output)

        #target_output_2 = self.dropout(definition_output)

    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            if len(line) == 8:
                ...
                examples.append(
                    InputExample(
                        guid=guid, text_a=text_a, text_b=index, label=label, POS=POS, FGPOS=FGPOS, text_a_2=text_a_2
                    )
                )
        return examples
...
def convert_examples_to_features(
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        # set the sentence as 5 in segment ids
        try:
            tokens_b += 1  # add 1 to the target word index considering [CLS]
            for i in range(len(text_b)):
                segment_ids[tokens_b + i] = 1
...
def convert_examples_to_two_features(
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        if args.use_local_context:
            ...
            # segment_ids_sent = [ # This line may be needed, but I don't know (by Takeshita)
            #     2 if i >= local_start and i <= local_end else 0 for i in range(len(tokens))
            # ]
        else:
            segment_ids = [0] * len(tokens)
            # segment_ids_sent = [0]*len(tokens)
        ...
        # # set the sentence as -1 in segment ids
        # try:
        #     for i in range(len(tokens_a)):
        #         segment_ids_sent[i+1] = -1 #add 1 to index considering [CLS]
        # except TypeError:
        #     pass
     ...
     #   text_b = tokenizer.tokenize(example.text_a_2)
        tok = tokenizer.tokenize(example.text_a_2)
